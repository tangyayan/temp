...
class BiLSTM_att(nn.Module):
    ...
    def forward(self, x, seq_lens=None):
        ...
        # H, _ = self.lstm(emb)  # [batch_size, L, hidden_size * 2]
        ...
        M = self.tanh1(H)  # [batch_size, L, hidden_size * 2]
        scores = self.w(M).squeeze(-1) # [batch_size, L]
        scores = scores.masked_fill(~mask, float("-inf"))

        alpha = F.softmax(scores, dim=1).unsqueeze(-1)  # [batch_size, L, 1]
        out = torch.sum(H * alpha, 1)  # [batch_size, hidden_size * 2]
        out = F.relu(out)
        out = self.fc1(out)
