...
项目地址：[tangyayan/temp](https://github.com/tangyayan/temp)
...
├── mydatasets/
│   ├── tnews_public/        # 用于存放原始 TNEWS 数据
│   ├── tnews_cleared/       # 用于存放清洗后的数据
│   ├── jieba/               # jieba 分词后的数据、词表和词向量（为减轻文件大小仅存储了词向量）
│   ├── char/                # 字符级切分后的数据、词表和词向量（为减轻文件大小仅存储了词向量）
...
需要先下载官方数据集到 `mydatasets/` 下。

如果需要从原始数据重新生成处理后的数据，可以依次运行：

```bash
python -m mydatasets.clear_data
python -m mydatasets.jieba_data
```
...
目前 `resulst/` 为使用 **Transformer+CLS+Jieba** 分词的结果。
...
```bash
python -m plot --model_name Transformer_jieba_sogou_cls --result_dir ./result
```

## 快速测试

在 `eval.py` 提供了测试训练好的模型（为Transformer+CLS+Jieba分词）的脚本：

```bash
python -m eval
```

运行后会输出该模型的分类报告、准确率和Macro-F1值，并写入文件 `test_results.json`
