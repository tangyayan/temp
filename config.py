...
class Config(object):
    ...
    def __init__(self, dataset, embedding, dataset_method):
        ...
        # output
        self.save_dir = f'./result/{self.model_name}'                  # 模型权重与结果保存目录
        ...
        self.patience = 1000                                            # 若超过1000batch F1还没提升，则提前结束训练
        self.grad_clip = 5.0                                            # 梯度裁剪阈值

        # train_print
        self.print_step = 5                                            # 每多少步打印一次训练状态
