...
OUTPUT_DIR     = "./mydatasets/char"
MIN_FREQ       = 1          # 低频词阈值（训练集词频 < MIN_FREQ 则为低频词）
...
def tokenize(text: str) -> list[str]:
    ...
    for i, part in enumerate(parts):
        if part:
            tokens.extend(list(part.replace(" ", "")))  # 去除空格后每个字符作为一个 token
        if i < len(parts) - 1:
            tokens.append(NUM_TOKEN)  # 在数字位置插入 NUM_TOKEN
    return tokens
